...
const renderApp = () => {
    ...
    if (loadingAuth) {
        content = `<div class="loading-screen">Loading DayLens...</div>`;
    } else if (!currentUser) {
    ...
    // Append a temporary message to the content if one is active
    if (message) {
        content += `<div class="fixed bottom-4 right-4 p-3 rounded-md shadow-lg text-white ${message.includes('Error') ? 'bg-red-500' : 'bg-green-500'}">
                        ${message}
                    </div>`;
    }

    // Append modals to the content if they are active
    if (showForm) {
        content += renderLogFormModal();
        // Attach event listeners for dynamically added "Add Entry" buttons
    ...
    if (showConfirmModal) {
        content += renderConfirmModal();
    }

    appRoot.innerHTML = content; // Update the DOM with the generated HTML
